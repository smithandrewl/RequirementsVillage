...
type IProjectRepository =
    abstract member GetAllAsync: unit -> Async<Result<Project list, ProjectError>>
    abstract member GetByIdAsync: Guid -> Async<Result<Project option, ProjectError>>
    abstract member CreateAsync: Project -> Async<Result<unit, ProjectError>>
    abstract member UpdateAsync: Project -> Async<Result<unit, ProjectError>>
    abstract member DeleteAsync: Guid -> Async<Result<unit, ProjectError>>
...
module Queries =
    let selectAll = """
        SELECT Id, Name, Description, Category, Status, CreatedAt, UpdatedAt
        FROM Projects
    ...
    let insert = """
        INSERT INTO Projects (Id, Name, Description, Category, Status, CreatedAt, UpdatedAt)
        VALUES (@Id, @Name, @Description, @Category, @Status, @CreatedAt, @UpdatedAt)
    """
...
type ProjectRepository(connectionString: string) =
    ...
    let executeAsync (operation: string) (action: IDbConnection -> Async<'T>) : Async<Result<'T, ProjectError>> =
        async {
            try
    ...
    member _.InitializeAsync() =
        executeAsync "CREATE_TABLE" (fun conn ->
            async {
                conn.Open()
                let! _ = conn.ExecuteAsync(Queries.createTable) |> Async.AwaitTask
                return ()
            })
    ...
    interface IProjectRepository with
        member _.GetAllAsync() =
            executeAsync "SELECT_ALL" (fun conn ->
                async {
                    conn.Open()
                    let! results = conn.QueryAsync<Project>(Queries.selectAll) |> Async.AwaitTask
        ...
        member _.GetByIdAsync(id: Guid) =
            executeAsync "SELECT_BY_ID" (fun conn ->
                async {
                    ...
                    let! result = conn.QuerySingleOrDefaultAsync<Project>(Queries.selectById, parameters) |> Async.AwaitTask
                    return 
        ...
        member _.CreateAsync(project: Project) =
            executeAsync "INSERT" (fun conn ->
                async {
                    ...
                    |}
                    let! _ = conn.ExecuteAsync(Queries.insert, parameters) |> Async.AwaitTask
                    return ()
                })
            ...
            executeAsync "UPDATE" (fun conn ->
                async {
                    ...
                    let parameters = {|
                        ...
                        UpdatedAt = project.UpdatedAt
                    |}
                    let! rowsAffected = conn.ExecuteAsync(Queries.update, parameters) |> Async.AwaitTask
                    if rowsAffected = 0 then
                        return! async { return raise (Exception("Project not found")) }
                    else
                        return ()
                })
        ...
        member _.DeleteAsync(id: Guid) =
            executeAsync "DELETE" (fun conn ->
                async {
                    conn.Open()
                    let parameters = {| Id = id.ToString() |}
                    let! _ = conn.ExecuteAsync(Queries.delete, parameters) |> Async.AwaitTask
                    return ()
                })
        ...
        member _.UpdateAsync(project: Project) =
            async {
                ...
                | Some idx ->
                    ...
                    return Ok ()
                | None ->
                    return Error (NotFound(project.Id, "InMemoryRepository"))
            }
