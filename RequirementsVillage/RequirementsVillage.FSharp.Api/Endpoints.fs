...
module ErrorHandlers =
    let handleProjectError (error: ProjectError) : HttpHandler =
        match error with
        | NotFound (id, context) ->
            RequestErrors.NOT_FOUND (json {| error = sprintf "Project %A not found in %s" id context |})
        | ValidationFailed (field, reason, value) ->
            RequestErrors.BAD_REQUEST (json {| error = sprintf "%s validation failed: %s" field reason; field = field; value = value |})
        | DatabaseError (operation, table, ex) ->
            ServerErrors.INTERNAL_ERROR (json {| error = sprintf "Database error in %s on %s" operation table |})
    ...
    let getProjects : HttpHandler =
        fun (next: HttpFunc) (ctx: HttpContext) ->
            task {
                ...
                let! result = service.GetAllProjectsAsync() |> Async.StartAsTask
    ...
    let getProject (id: string) : HttpHandler =
        fun (next: HttpFunc) (ctx: HttpContext) ->
            task {
                ...
                | true, guid ->
                    let service = ctx.GetService<IProjectService>()
                    let! result = service.GetProjectByIdAsync(guid) |> Async.StartAsTask
                    
                    match result with
                    | Ok (Some project) ->
                        return! json project next ctx
                    | Ok None ->
                        return! RequestErrors.NOT_FOUND (json {| error = sprintf "Project %s not found" id |}) next ctx
                    | Error error ->
                        return! handleProjectError error next ctx
                | false, _ ->
                    return! RequestErrors.BAD_REQUEST (json {| error = "Invalid project ID format" |}) next ctx
            }
    ...
    let createProject : HttpHandler =
        fun (next: HttpFunc) (ctx: HttpContext) ->
            task {
                ...
                let! result = service.CreateProjectAsync(request.Name, request.Description, category) |> Async.StartAsTask
                ...
                | true, guid ->
                    ...
                    | Ok status ->
                        let project = {
                            ...
                            Status = status
                            CreatedAt = DateTime.MinValue // Will be ignored by service
                            UpdatedAt = DateTime.UtcNow
                        }
                        
                        let! result = service.UpdateProjectAsync(project) |> Async.StartAsTask
                        ...
                        | Error error ->
                            return! handleProjectError error next ctx
                    | Error msg ->
                        return! RequestErrors.BAD_REQUEST (json {| error = msg |}) next ctx
                | false, _ ->
                    return! RequestErrors.BAD_REQUEST (json {| error = "Invalid project ID format" |}) next ctx
            }
        ...
        fun (next: HttpFunc) (ctx: HttpContext) ->
            task {
                ...
                | true, guid ->
                    ...
                    match parseStatus request.Status with
                    | Ok status ->
                        let! result = service.UpdateProjectStatusAsync(guid, status) |> Async.StartAsTask
    ...
    let deleteProject (id: string) : HttpHandler =
        fun (next: HttpFunc) (ctx: HttpContext) ->
            task {
                ...
                | true, guid ->
                    let service = ctx.GetService<IProjectService>()
                    let! result = service.DeleteProjectAsync(guid) |> Async.StartAsTask
    ...
    choose [
        ...
        subRoute "/api/projects" (
            choose [
                ...
                PATCH >=> routef "/%s/status" ProjectEndpoints.updateProjectStatus
