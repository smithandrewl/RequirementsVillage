...
let view (model: Model) (dispatch: Msg -> unit) =
    ...
    Html.div [
        Html.div [
            prop.className "is-flex is-flex-wrap-wrap is-align-items-center is-justify-content-space-between"
            prop.style [ style.marginBottom (length.rem 1.5) ]
            prop.children [
                Bulma.select [
                    prop.onChange (fun (e: Browser.Types.Event) ->
                        let value = (e.target :?> Browser.Types.HTMLSelectElement).value
                        let filter = 
                            match value with
                            | "All" -> None
                            | "Idea" -> Some Idea
